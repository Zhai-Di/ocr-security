...
if __name__ == "__main__":
    ...
    traces_file_path = "../data/ens_case_data_paper2.csv"
    oracles = 31
    type_str = "deflation"
    ...
    scenario_1_result, scenario_2_result = calculate_accumulated_deviations(obs_file_path, traces_file_path,
                                                                            oracles, type_str)
    scenario_1_result_usd = scenario_1_result * eth_usd_price
    scenario_1_result = scenario_1_result.quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)
    scenario_1_result_usd = scenario_1_result_usd.quantize(Decimal("1"), rounding=ROUND_HALF_UP)
    scenario_2_result_usd = scenario_2_result * eth_usd_price
    scenario_2_result = scenario_2_result.quantize(Decimal("0.0001"), rounding=ROUND_HALF_UP)
    scenario_2_result_usd = scenario_2_result_usd.quantize(Decimal("1"), rounding=ROUND_HALF_UP)
    print(f"Experimental results for the deflation case, which show an increase in ENS's revenue.")
    print(f"scenario 1 revenue deviation (ETH)   {'+' + str(scenario_1_result)} ETH")
    print(f"scenario 1 revenue deviation (USD)   {'+' + str(scenario_1_result_usd)} USD")
    print(f"scenario 2 revenue deviation (ETH)   {'+' + str(scenario_2_result)} ETH")
    print(f"scenario 2 revenue deviation (USD)   {'+' + str(scenario_2_result_usd)} USD")